...
class Trainer(object):
    def __init__(self, cfg, model, train_dl, val_dl, criterion):
        ...
        resume_ckpt = None
        if isinstance(self.cfg.RESUME, str) and self.cfg.RESUME:
            resume_ckpt = self.cfg.RESUME
        elif self.cfg.RESUME and self.cfg.RESUME_PATH:
            resume_ckpt = self.cfg.RESUME_PATH

        if resume_ckpt:
            checkpoint = torch.load(resume_ckpt, map_location='cpu')
            self.model_without_ddp.load_state_dict(checkpoint['model'])
            if not self.cfg.test and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
    ...
    def handle_new_epoch(self):
        ...
        if self.train_epoch % self.eval_period == 0 and self.train_epoch != 0:
            ...
            print(
                'VAL epoch={} mae={:.2f} mse={:.2f} best_mae={:.2f}'.format(
                    self.train_epoch,
                    result[0],
                    result[1],
            ...
            if result[0] < self.best_mae - 1e-6:
                self.best_mae = result[0]
                self.no_improve_epochs = 0
            else:
            ...
            if self.no_improve_epochs >= self.early_stop_patience:
                ...
                self.logger.info(
                    'EARLY STOP at epoch=%d (patience=%d, best_mae=%.2f)',
                    self.train_epoch,
