...
class ImageDataset(Dataset):
    def __init__(self, data_root, transform=None, train=False, aug_dict=None,
                 dataset_name='SHHA', train_list='', eval_list=''):
        self.root_path = data_root
        self.train_lists = self._resolve_list_name(train_list, 'train', dataset_name, 'train.list')
        self.eval_list = self._resolve_list_name(eval_list, 'test', dataset_name, 'test.list')
    ...
    def _resolve_list_name(self, preferred, split, dataset_name, default_list):
        candidates = []
        if preferred:
            candidates.append(preferred)

        candidates.append(default_list)

        ds = str(dataset_name).upper()
        if ds == 'SHHA':
            candidates.append('shanghai_tech_part_a_{}.list'.format(split))
        elif ds == 'SHHB':
            candidates.append('shanghai_tech_part_b_{}.list'.format(split))

        for name in candidates:
            if os.path.isfile(os.path.join(self.root_path, name)):
                return name

        raise FileNotFoundError(
            'Cannot find {} list under {}. Tried: {}'.format(
                split,
                self.root_path,
                ', '.join(candidates),
            )
