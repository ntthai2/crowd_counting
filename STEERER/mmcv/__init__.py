"""
Minimal mmcv shim providing Config and DictAction for STEERER.
mmcv 1.6.0 is incompatible with Python >=3.12 (uses removed pkgutil.ImpImporter).
This shim replicates the subset of mmcv used by STEERER tools.
"""
import argparse
...
class Config:
    ...
    def merge_from_dict(self, options):
        """Merge a flat dict of overrides (supports dot-separated keys)."""
        if not options:
            return
...
class DictAction(argparse.Action):
    ...
    Supports: key=value, nested.key=value, key=[v1,v2], key=(v1,v2).
    """
