...
parser.add_argument('--epochs', type=int, default=400,
                    help='total number of epochs to train (default: 400)')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        prec1, val_mse = validate(val_list, model, criterion)
        
        is_best = prec1 < best_prec1
        best_prec1 = min(prec1, best_prec1)
        print(f'VAL epoch={epoch} mae={float(prec1):.2f} mse={float(val_mse):.2f} best_mae={float(best_prec1):.2f}')
        save_checkpoint({
            'epoch': epoch + 1,
            'arch': args.pre,
        ...
        if epochs_no_improve >= args.patience:
            print(f'Early stopping: val MAE has not improved for {args.patience} epochs. Best MAE: {best_prec1:.3f}')
            break
...
def validate(val_list, model, criterion):
    ...
    for i,(img, target) in enumerate(test_loader):
        ...
        diff = output.data.sum() - target.sum().type(torch.FloatTensor).cuda()
        mae += abs(diff)
        mse += diff * diff
