...
def loading_data_shhb(data_root):
    """Same as loading_data but uses SHB list files."""
    transform = standard_transforms.Compose([
        standard_transforms.ToTensor(),
        standard_transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                      std=[0.229, 0.224, 0.225]),
    ...
    train_set = SHHA(data_root, train=True, transform=transform, patch=True, flip=True,
                     train_list="shanghai_tech_part_b_train.list",
                     eval_list="shanghai_tech_part_b_test.list")
    val_set = SHHA(data_root, train=False, transform=transform,
                   train_list="shanghai_tech_part_b_train.list",
                   eval_list="shanghai_tech_part_b_test.list")
    return train_set, val_set
