...
def main(args):
    ...
    train_file, test_file = dataset_map[args['dataset']]
    ...
    if args['distributed']:
        ...
        log_path = './save_file/log_file/' + time.strftime("%Y%m%d_%H%M%S", time.localtime(time.time()))
        if not os.path.exists(args['save_path']) and args['local_rank'] == 0:
            os.makedirs(args['save_path'])
        if args['save']:
            logger = get_root_logger(log_path + '/1.log')
        else:
        ...
        args['local_rank'] = 0  # treat as rank-0 for logging in single-GPU mode
        model = nn.DataParallel(model, device_ids=[0])
        log_path = './save_file/log_file/debug/'
        if not os.path.exists(args['save_path']):
            os.makedirs(args['save_path'])
        logger = get_root_logger(log_path + 'debug.log')
        writer = SummaryWriter(log_path)
